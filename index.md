...
確率密度関数 $\rho(x)$ が決まったので，累積分布関数が決まるはず...なのだけど， $x$ は多次元のベクトルであって実数ではないから，単純に「 $x \leq x_0$ 」のような大小関係が存在しないので，「 $-\infty$ から $x_0$ まで積分」みたいなこともできない．ゆえに「累積分布関数」に相当するものはないとも言える．ただし，「どこかからどこまで累積させる」が言えればよいわけだから，「 $\Gamma$ の特定の領域について積分する」ということはできる．
